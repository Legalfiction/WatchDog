...
export default function App() {
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-slate-50 font-sans text-slate-900 flex flex-col">
      {/* Animatie-stijl voor de stuiterende Zzz */}
      <style>{`
        @keyframes bounce-zz {
        ...
        <main className="flex-1 p-6 flex flex-col items-center justify-start pt-16 space-y-12">
          ...
          <button 
            ...
            className={`relative w-72 h-72 rounded-full flex flex-col items-center justify-center transition-all duration-500 shadow-2xl active:scale-95 group overflow-hidden border-[1px] ${
              ...
              settings.vacationMode ? 'border-blue-500/30' : 'border-orange-500/30'
            }`}
          >
            {status !== 'connected' ? (
              ...
              <div className="relative w-full h-full bg-slate-900 flex flex-col items-center justify-center p-8">
                <img src={IMG_SLEEPING} alt="Slapende Hond" className="w-44 h-44 object-contain mb-6 drop-shadow-[0_0_15px_rgba(30,58,138,0.5)]" />
                
                {/* STUITERENDE ZZ TEKENS */}
                <div className="absolute top-16 right-16 flex font-black text-blue-300 pointer-events-none">
                  <span className="text-2xl animate-zz" style={{animationDelay: '0s'}}>Z</span>
                  <span className="text-xl animate-zz ml-1" style={{animationDelay: '0.4s'}}>z</span>
                  <span className="text-lg animate-zz ml-1" style={{animationDelay: '0.8s'}}>z</span>
                </div>

                <span className="text-sm font-black uppercase text-blue-200 tracking-widest bg-slate-800/50 px-4 py-2 rounded-full border border-blue-500/20">
                  Wakker worden
                </span>
              </div>
            ) : (
              /* --- ACTIEVE STAND (ORANJE) --- */
              <div className="relative w-full h-full bg-orange-600 flex flex-col items-center justify-center p-8">
                 <img src={IMG_BARKING} alt="Blaffende Hond" className="w-48 h-48 object-contain mb-6 drop-shadow-[0_10px_20px_rgba(0,0,0,0.2)]" />
                 <span className="text-sm font-black uppercase text-white tracking-widest bg-orange-500/50 px-4 py-2 rounded-full border border-white/20">
                   Tik om te slapen
                 </span>
              </div>
            )}
          ...
          {/* VERBINDINGSLABEL ONDERAAN (VERPLAATST VOOR OVERLAP) */}
          {activeUrl && status === 'connected' && (
            <div className={`mt-4 px-6 py-2.5 rounded-full text-[11px] font-bold uppercase flex items-center gap-2 shadow-sm border ${
              settings.vacationMode ? 'bg-slate-800 text-slate-300 border-slate-700' : 'bg-white text-orange-600 border-orange-100'
            }`}>
